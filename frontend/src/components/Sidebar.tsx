...
export default function Sidebar() {
    ...
    <>
      ...
      <div className="p-6 border-b border-slate-100 dark:border-slate-700">
        <Link href="/dashboard" className="flex items-center gap-3" onClick={() => setOpen(false)}>
          <div className="w-10 h-10 bg-brand-500 rounded-2xl flex items-center justify-center text-white font-black text-lg">iT</div>
          <div>
      ...
      <nav className="flex-1 overflow-y-auto p-4 space-y-6">
        <div>
          ...
          <div className="space-y-1">{principalLinks.map(link => (
            <SidebarLink key={link.href} href={link.href} label={link.label} icon={link.icon} active={isActive(link.href)} onNavigate={() => setOpen(false)} />
          ))}</div>
        </div>

        {isGerente && (
          <div>
            <button onClick={() => setAdminExpanded(!adminExpanded)} className="flex items-center justify-between w-full px-3 mb-2">
            ...
            {adminExpanded && <div className="space-y-1">{adminLinks.map(link => (
              <SidebarLink key={link.href} href={link.href} label={link.label} icon={link.icon} active={isActive(link.href)} onNavigate={() => setOpen(false)} />
            ))}</div>}
          </div>
        )}
      ...
      <div className="p-4 border-t border-slate-100 dark:border-slate-700">
        {user && (
          <div className="flex items-center justify-between">
            ...
            <button onClick={handleLogout} className="w-9 h-9 bg-slate-100 dark:bg-slate-700 rounded-xl flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-rose-50 hover:text-rose-500 transition-colors shrink-0" title="Sair">
  ...
  return (
    <>
      {/* Mobile toggle */}
      <button onClick={() => setOpen(!open)} className="fixed top-4 left-4 z-50 md:hidden w-12 h-12 bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-700 dark:text-slate-300">
      ...
      {/* Mobile sidebar */}
      <div className={`fixed top-0 left-0 z-40 w-72 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 transition-transform duration-300 flex flex-col md:hidden ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        {sidebarContent}
      </div>
...
function SidebarLink({ href, label, icon: Icon, active, onNavigate }: {
  ...
  return (
    <Link href={href} onClick={onNavigate} className={`flex items-center gap-3 px-3 py-2.5 rounded-2xl transition-all text-sm font-bold ${
      active ? 'bg-brand-500 text-white shadow-lg shadow-brand-200 dark:shadow-none' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700/50 hover:text-slate-900 dark:hover:text-white'
    }`}>
