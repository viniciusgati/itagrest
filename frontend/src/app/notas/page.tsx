...
import { FileText, CheckCircle2, AlertCircle, Search, Printer } from 'lucide-react'
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-700">
                {notas.map((nota: any) => (
                  <tr key={nota.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors group">
                    ...
                    <td className="p-6 text-sm text-slate-600 dark:text-slate-400 font-medium">{nota.protocolo || '-'}</td>
                    <td className="p-6">
                      <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${
                        nota.status_sefaz === '100' ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400' : 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400'
                      }`}>
